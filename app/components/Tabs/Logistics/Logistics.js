...
export default class Logistics extends React.Component {
      ...
      addTab: [
        {
          ...
          render: () => (
            <Tab.Pane>
              <div className="addTab">Add Your Venue/Caterer</div>
            </Tab.Pane>
          )
        }
      ],
  ...
  deleteTab = index => {
    this.props.socket.emit("deleteLogisticsTab", {
      eventId: this.props.eventId,
      index: index
    });
  };
  componentDidMount = () => {
    ...
    this.props.socket.on("addLogisticsTab", data => {
      ...
      if (data.newTab.vORp === "caterer") {
        addRender = (
          <Food
            key={updateTabs.length}
            title={data.newTab.title}
            eventId={this.props.eventId}
            socket={this.props.socket}
            tabIndex={updateTabs.length}
          />
        );
      } else {
        addRender = (
      ...
      updateTabs.push({
        menuItem: {
          key: data.newTab.title,
          content: (
            <Header as="h4">
              <Header.Content>
                {data.newTab.title} &emsp;
                <Icon
                  color="grey"
                  name="cancel"
                  onClick={() => this.deleteTab(updateTabs.length)}
                />{" "}
              </Header.Content>
            </Header>
          )
